...
describe('support flows package', () => {
  ...
  it(
    'handles the customer support flow for tickets, knowledge-base search, and live chat',
    async () => {
  ...
  it(
    'lets an admin triage and resolve tickets created through the shared adapter',
    async () => {
      const user = userEvent.setup();
      const adapter = createInMemorySupportFlowAdapter();
      ...
      await user.click(
        screen.getByRole('button', { name: 'Reply to customer' })
      );
      await user.type(
        screen.getByPlaceholderText(
